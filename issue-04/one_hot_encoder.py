...
def test_fit_transform_equal():
    ...
    """
    assert fit_transform(['Moscow', 'Tula']) == [('Moscow', [0, 1]), ('Tula', [1, 0])]


def test_fit_transform_type():
    """
    Проверка на тип возвращаемого
    """
    assert type(fit_transform(['Moscow'])) == list
...
def test_fit_transform_in():
    ...
    """
    expected = ('Moscow', [0, 1])
    assert expected in fit_transform(['Moscow', 'Tula'])
